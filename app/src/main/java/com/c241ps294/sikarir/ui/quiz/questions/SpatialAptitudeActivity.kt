        ...
        onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                if (questionPointer == 0) {
                    val intent = Intent(this@SpatialAptitudeActivity, NumericalAptitudeActivity::class.java)
                    intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT)
                    startActivity(intent, ActivityOptions.makeCustomAnimation(this@SpatialAptitudeActivity, 0, 0).toBundle())
                } else {
                    questionPointer--
                    bindViews()
                }
